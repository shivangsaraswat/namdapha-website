...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[9999] w-full px-6 py-5 md:px-8 lg:px-12 text-white transition-transform duration-500 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'
      } max-md:[body[data-dialog-open]_&]:hidden`}>
      <nav className={`flex items-center justify-between mx-auto relative transition-all duration-300 ${isScrolled ? 'max-w-[900px] lg:max-w-[1000px]' : 'max-w-[1200px]'
        }`}>
        {/* Glassmorphism Background */}
        <div
          className={`absolute -left-4 -right-4 -top-2 -bottom-2 rounded-[2rem] transition-all duration-300 ${isScrolled ? 'opacity-100' : 'opacity-0'
            }`}
          style={{
            zIndex: -1,
        ...
        <div className={`flex items-center relative z-10 transition-all duration-300 ${isScrolled ? 'space-x-2' : 'space-x-3'
          }`}>
        ...
        {/* Desktop Navigation */}
        <div className={`hidden lg:flex items-center rounded-full px-4 py-2 relative z-10 transition-all duration-300 ${isScrolled ? 'border border-white/20 space-x-1' : 'border border-transparent space-x-3'
          }`} style={{
